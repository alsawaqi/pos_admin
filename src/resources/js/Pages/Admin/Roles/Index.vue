...
/**
 * Admin Roles & Permissions — the platform-side role builder.
 *
 * Phase 4.8b. Mirrors pos_merchant's Roles/Index.vue with the
 * platform permission catalog. Manages the 5 default platform
 * roles (editable but undeletable) + any custom roles built
 * by the SuperAdmin.
 *
 * Permission gating:
 *   - Page reachable when PlatformPermission.RolesView
 *   - Create / Edit / Delete buttons only when RolesManage
 */
...
const canManage = computed(() => can(PlatformPermission.RolesManage));
</script>
...
<template>
    <AdminLayout>
        <section class="space-y-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                ...
                <button
                    v-if="canManage"
                    type="button"
                    class="inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-600/30 transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-indigo-600/40"
                    @click="openCreate"
                >
                    <Plus class="size-4" />
            ...
            <section v-else class="space-y-4">
                <article
                    v-for="role in roles"
                    :key="role.id"
                    class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-indigo-200"
                >
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <label class="block">
                        ...
                        <input
                            ...
                            class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-4 focus:ring-indigo-100 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-500"
                        >
                    ...
                    <label class="block">
                        ...
                        <input v-model="editorForm.description" type="text" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-4 focus:ring-indigo-100">
                    </label>
                </div>
        ...
        <!-- =============== DELETE CONFIRM =============== -->
        <BaseModal v-if="deleteTarget" :title="t('roles.delete_dialog.title')" size="md" :loading="deleting" @close="deleteTarget = null">
