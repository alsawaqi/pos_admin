 ...
 * password and a real session (+ JWT cookie) for a TOTP-enrolled
 * platform admin.
 *
 ...
 * exactly like the normal flow — we hard-navigate to /admin so the
 * SPA re-bootstraps off the fresh session + JWT.
 *
 * A visit with no pending challenge (deep link, expired state)
 * bounces back to /login.
 */
import { KeyRound, Loader2, ShieldCheck, Smartphone, TriangleAlert } from 'lucide-vue-next';
...
async function onSubmit(): Promise<void> {
    ...
    try {
        ...
        // Full reload — the fresh session + JWT bootstrap
        // __INITIAL_AUTH__ for the admin shell.
...
<template>
    <main class="grid min-h-screen place-items-center bg-slate-50 px-5 py-10 text-slate-950">
        <div class="w-full max-w-md">
            <div class="rounded-lg border border-slate-200 bg-white p-7 shadow-2xl shadow-slate-200/70 sm:p-8">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-2xl font-semibold tracking-tight">{{ t('auth.two_factor.title') }}</h2>
                ...
                <template v-else-if="expired">
                    <div class="mt-8 grid place-items-center">
                        <span class="grid size-14 place-items-center rounded-lg bg-amber-50 text-amber-600">
                            <TriangleAlert class="size-7" />
                        </span>
                    </div>
                    <h3 class="mt-6 text-center text-xl font-semibold tracking-tight">
                    ...
                    <RouterLink
                        to="/login"
                        class="mt-8 inline-flex w-full items-center justify-center rounded-lg bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800"
                    >
                ...
                <template v-else>
                    ...
                    <form class="mt-8 space-y-5" @submit.prevent="onSubmit">
                        ...
                        <label v-if="!useRecovery" class="block">
                            ...
                            <div class="group relative mt-2">
                                ...
                                <input
                                    ...
                                    class="w-full rounded-lg border border-slate-200 bg-white ps-11 pe-4 py-3 text-center text-lg font-bold tracking-[0.4em] text-slate-950 shadow-sm transition focus:border-teal-500 focus:outline-none focus:ring-4 focus:ring-teal-100"
                                >
                            </div>
                        </label>
                        ...
                        <label v-else class="block">
                            ...
                            <div class="group relative mt-2">
                                ...
                                <input
                                    ...
                                    class="w-full rounded-lg border border-slate-200 bg-white ps-11 pe-4 py-3 text-center text-sm font-bold uppercase tracking-widest text-slate-950 shadow-sm transition focus:border-teal-500 focus:outline-none focus:ring-4 focus:ring-teal-100"
                                >
                            </div>
                        </label>
                        ...
                        <button
                            type="submit"
                            :disabled="submitting"
                            class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-wait disabled:opacity-70"
                        >
