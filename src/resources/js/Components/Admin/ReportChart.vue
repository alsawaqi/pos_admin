...
/**
 * ReportChart — admin port of the merchant v2 chart wrapper (ApexCharts).
 *
 * One reusable component for every admin report/dashboard graph so the
 * theme (palette, fonts, tooltip, grid) stays uniform. Wraps
 * `vue3-apexcharts` and applies the slate/teal house style.
 *
 * Supported `type`s: 'bar' (vertical/horizontal), 'line', 'area',
 * 'donut'. For bar/line/area pass `series` as ApexAxisChartSeries
 * ([{ name, data:number[] }]) plus `categories` (x labels). For donut
 * pass `series` as number[] plus `labels` (slice names).
 *
 * Money series are decimal-OMR (report actions emit decimal-3 strings —
 * parse before passing). Set `currency` so tooltips/axis append "OMR"
 * and group thousands. Chart numerals stay Latin even in Arabic locale.
 */
...
// House palette — teal-led, matches the admin UI accents.
const PALETTE = [
