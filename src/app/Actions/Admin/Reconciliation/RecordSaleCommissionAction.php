...
/**
 * TWIN of pos_api app/Actions/Device/Sync/RecordSaleCommissionAction.php —
 * keep in sync. (The repos share the DB but not code; this duplication is
 * the accepted cost. P-F7: pos_api SKIPS the split at order.pay when a
 * tender is pending_reconciliation; this twin records the identical rows
 * once the admin's approval confirms the money actually arrived.)
 *
 ...
 *
 * Money is split in integer baisas so it never drifts: each non-merchant
 * party is rounded, and the MERCHANT takes the exact remainder, so Σ(rows)
 * == grand_total to the baisa. The profile + percents are SNAPSHOT onto
 * every row, so later edits to the merchant's profile never rewrite this
 * sale.
 *
 ...
 * this is what makes approval replays / double-clicks safe.
 *
