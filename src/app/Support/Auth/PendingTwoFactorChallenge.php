 ...
 * platform admin (Phase D8). Mirror of pos_merchant's class —
 * different session-key prefix + config namespace.
 *
 * After the password check passes, the login controller does NOT
 * authenticate and does NOT issue the JWT cookie — it parks
 * {user id, remember-me intent, expiry} here and bounces the
 * browser to /two-factor-challenge. Only the challenge endpoint
 * can convert this pending state into a real session + JWT, so a
 * stolen password alone never yields auth.
 */
final readonly class PendingTwoFactorChallenge
{
    private const KEY_USER_ID = 'pos_admin.two_factor.pending_user_id';

    private const KEY_REMEMBER = 'pos_admin.two_factor.remember';

    private const KEY_EXPIRES_AT = 'pos_admin.two_factor.expires_at';
